...
setwd(
  "~/Library/CloudStorage/Dropbox/phd/Revised_descriptive_7.11.23/Revised_descriptive_brucellosis"
)
...
human_zoonoses <- human_zoonoses %>%
  ...
  mutate(diseases = ifelse(str_detect(diseases, "Brucella|brucellosis"), "Brucellosis", diseases))
  ...
  mutate(diseases = ifelse(
    str_detect("Brucella|brucellosis", diseases),
    "Brucellosis",
    diseases
  ))
...
Animaldata <- read_csv("animal_data1.csv") %>%
  ...
  dplyr::select(
    ...
    month_2,
    disease_condition,
    nature_of_diagnosis_clinical_lab_pm,
    species,
    no_sick
  )
...
Animaldata_21 <- read_csv("july-dec_animal.xlsx.csv") %>%
  janitor::clean_names() %>%
  rename(
    nature_of_diagnosis_clinical_lab_pm = nature_of_diagnosis,
    species = species_affected,
    year_1 = year_10
  ) %>%
  dplyr::select(
    county,
...
animal_zoonoses <- animal_zoonoses_comb %>%
  ...
  mutate(disease_condition = ifelse(
    grepl("Rabies|Rbies", disease_condition),
    "Rabies",
    ifelse(
      grepl("Brucell", disease_condition),
      "Brucellosis",
  ...
  mutate(diagnosis = ifelse(
    ...
    "Lab confirmed",
    ifelse(
      diagnosis %in% c("Pm", "Postmortem", "Post Mortem"),
      "Post Mortem",
      "Clinically confirmed"
    )
  )) %>%
  ...
  mutate(species = ifelse(
    ...
    ifelse(
      ...
      "Buffalo",
      ifelse(
        species %in% c("Cani", "Canine(Jackal)", "Cannine", "Dog", "Dogs"),
        "Canine",
        ifelse(
          species %in% c("Goat"),
          "Goats",
          ifelse(species %in% c("", "Nd"), NA, species)
        )
      )
    )
  )) %>%
  mutate(
    species = recode(
      species,
      "Bovine & Canine" = "Canine",
      "Caprine/Ovine" = "Caprine",
      "Caprine+Donkey" = "Caprine",
      "Dogs/Donkey" = "Dogs",
      "Canine, Feline" = "Dogs",
      "Dogs, Cats" = "Dogs"
    )
...
all_zoonoses <- rbind(human_zoonoses, animal_zoonoses) %>%
  mutate(diseases = ifelse(
    grepl("Brucella|brucellosis", diseases),
    "Brucellosis",
    diseases
  )) %>%
  mutate(county = ifelse(str_detect("Murang\xe1|Muranga", county), "Murang'a", county)) %>%
  mutate(
    species = recode(
      species,
      "Camel" = "Camels",
...
bruc_inc_data <- all_bruc_inci %>%
  ...
  ) |>  # Replace with your actual species columns
  names_to = "species",
  values_to = "incidence")
...
line_plots <-
  ggplot(melted_data, aes(x = date, y = incidence, color = species)) +
  geom_line() +
  labs(title = "Incidence Over Time by Species",
       x = "Date",
       y = "Incidence")
...
hum_ani_plot <-
  ggplot(brucellosis_data, aes(x = date, y = incidence_human)) + geom_line() +
...
ggsave(
  "humanincidences_ecozone_plot.png",
  human_inc_plot,
  width = 16,
  height = 10
)
...
# Line plots
human_inc_trend <-
  ggplot(all_bruc_inci, aes(x = y, y = hum_incidence)) +
  geom_smooth() +
  geom_point() +
  theme(text = element_text(size = 18)) +
  labs(x = "Livelihood Zones", y = "Human brucellosis incidence (annual Avg)")
...
for (species_col in species_columns) {
  plot <-
    ggplot(all_bruc_inci, aes(x = date, y = .data[[species_col]], color = species)) +
    geom_line() +
    labs(
  ...
  hum_inc_plot <-
    ...
    geom_line() +
    labs(title = "Incidence by Species",
         x = "Date",
         y = "Incidence")
  hum_inc_plot
  
  ggplot(all_bruc_inci, aes(x = date, y = all, fill = diseases)) + geom_col() +
    facet_grid(type ~ diagnosis, scales = "free_y") + theme_bw() +
    scale_fill_brewer(palette = "Set1") + labs(x = "Period", y = "Frequency", fill =
                                                 "Disease") + theme(text = element_text(size = 18)) +
  ...
  county <- st_read("County.shp") %>%
    mutate(COUNTY = recode(COUNTY, "Keiyo-Marakwet" = "Elgeyo Marakwet", "Tharaka" =
                             "Tharaka Nithi"))
  
  ##create anthrax population files for mapping (animals and human)
  county_anthrax1 <-
    full_join(county, all_zoonoses1b[all_zoonoses1b$diseases %in% "Anthrax" &
                                       all_zoonoses1b$type %in% "Human", ], by = c("COUNTY" = "county"))
  county_anthrax2 <-
    full_join(county, all_zoonoses1a[all_zoonoses1a$diseases %in% "Anthrax" &
                                       all_zoonoses1a$type %in% "Animal", ], by = c("COUNTY" = "county"))
  county_anthrax1$type <-
    ifelse(is.na(county_anthrax1$type), "Human", county_anthrax1$type)
  county_anthrax2$type <-
    ifelse(is.na(county_anthrax2$type),
           "Animal",
           county_anthrax2$type)
  ...
  anthrax_h <-
    ggplot(county_anthrax1, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid( ~
                                                                                      diseases) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
    ...
    labs(fill = "Average cases \n per 100,000 people") + theme(text = element_text(size =
                                                                                     16))
  ...
  county_brucellosis1 <-
    full_join(county, all_zoonoses1b[all_zoonoses1b$diseases %in% "Brucellosis" &
                                       all_zoonoses1b$type %in%
                                       "Human", ], by = c("COUNTY" = "county"))
  
  county_brucellosis1$diseases <- "Brucellosis"
  brucellosis_h <-
    ggplot(county_brucellosis1, aes(fill = inc)) + geom_sf() + theme_bw() +
    facet_grid( ~ diseases) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  animal_cases <- animal_zoonoses_comb %>%
    ...
    mutate(
      species = recode(
        species,
        "Bovine & Canine" = "Bovine , Canine",
        "Caprine/Ovine" = "Caprine,Ovine",
        "Caprine+Donkey" = "Caprine,Donkey",
        "Dogs/Donkey" = "Dogs,Donkey"
      )
    ) %>%
  ...
  animal_pop <- V4_T2.24 %>%
    ...
    pivot_longer(
      c("Sheep", "Goats", "Camels", "Donkeys", "Pigs", "cattle"),
      names_to = "species",
      values_to = "species_num"
    )
  ...
  animal_data <-
    left_join(animal_cases,
              animal_pop,
              by = c("county" = "County", "species" = "species")) %>%
    mutate(inc = ifelse(
      !is.na(species_num),
      disease_values / species_num * 100000,
      disease_values
    ))
  ...
  anthrax_buffalo <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Buffalo", ], by = c("COUNTY" = "county"))
  
  anthrax_camels <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Camels", ], by = c("COUNTY" = "county"))
  anthrax_canine <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Canine", ], by = c("COUNTY" = "county"))
  anthrax_cattle <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "cattle", ], by = c("COUNTY" = "county"))
  anthrax_goats <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Goats", ], by = c("COUNTY" = "county"))
  anthrax_pigs <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Pigs", ], by = c("COUNTY" = "county"))
  anthrax_rhino <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Rhino", ], by = c("COUNTY" = "county"))
  anthrax_sheep <-
    full_join(county, animal_data[animal_data$diseases %in% "Anthrax" &
                                    animal_data$species %in% "Sheep", ], by = c("COUNTY" = "county"))
  
  anthrax_buffalo$species <- "Buffalo"
  anthrax_buf <-
    ggplot(anthrax_buffalo, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                     species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_cml <-
    ggplot(anthrax_camels, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                    species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_cnine <-
    ggplot(anthrax_canine, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                    species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_cttl <-
    ggplot(anthrax_cattle, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                    species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_gts <-
    ggplot(anthrax_goats, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_pgs <-
    ggplot(anthrax_pigs, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                  species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_rhno <-
    ggplot(anthrax_rhino, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  anthrax_shp <-
    ggplot(anthrax_sheep, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  plot2 <-
    gridExtra::grid.arrange(anthrax_cnine,
                            anthrax_cttl,
                            anthrax_cml,
                            anthrax_gts,
                            anthrax_pgs,
  ...
  ggsave("animal_anthrax.png",
         plot2,
         width = 16,
         height = 9)
  ggsave("anthrax_human.png",
         anthrax_h,
         width = 7,
         height = 7)
  ...
  brucellosis_camels <-
    full_join(county, animal_data[animal_data$diseases %in% "Brucellosis" &
                                    animal_data$species %in% "Camels", ], by = c("COUNTY" = "county"))
  brucellosis_canine <-
    full_join(county, animal_data[animal_data$diseases %in% "Brucellosis" &
                                    animal_data$species %in% "Canine", ], by = c("COUNTY" = "county"))
  ...
  brucellosis_donkey <-
    full_join(county, animal_data[animal_data$diseases %in% "Brucellosis" &
                                    animal_data$species %in% "Donkeys", ], by = c("COUNTY" = "county"))
  ...
  brucellosis_cml <-
    ggplot(brucellosis_camels, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                        species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  brucellosis_cnine <-
    ggplot(brucellosis_canine, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                        species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  brucellosis_cttl <-
    ggplot(brucellosis_cattle, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                        species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  brucellosis_gts <-
    ggplot(brucellosis_goats, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                       species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  brucellosis_pgs <-
    ggplot(brucellosis_donkey, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                        species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  brucellosis_shp <-
    ggplot(brucellosis_sheep, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                       species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  ggsave("brucellosis_animals.png",
         plot2,
         width = 16,
         height = 9)
  ggsave("brucellosis_humans.png",
         brucellosis_h,
         width = 7,
         height = 7)
  ...
  rvf_camels <-
    full_join(county, animal_data[animal_data$diseases %in% "Rvf" &
                                    animal_data$species %in% "Camels", ], by = c("COUNTY" = "county"))
  ...
  rvf_cml <-
    ggplot(rvf_camels, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rvf_cttl <-
    ggplot(rvf_cattle, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rvf_gts <-
    ggplot(rvf_goats, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                               species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rvf_shp <-
    ggplot(rvf_sheep, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                               species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_camels <-
    full_join(county, animal_data[animal_data$diseases %in% "Rabies" &
                                    animal_data$species %in% "Camels", ], by = c("COUNTY" = "county"))
  rabies_feline <-
    full_join(county, animal_data[animal_data$diseases %in% "Rabies" &
                                    animal_data$species %in% "Feline", ], by = c("COUNTY" = "county"))
  rabies_canine <-
    full_join(county, animal_data[animal_data$diseases %in% "Rabies" &
                                    animal_data$species %in% "Canine", ], by = c("COUNTY" = "county"))
  rabies_cattle <-
  ...
  rabies_feline <-
    ggplot(rabies_feline, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_canine <-
    ggplot(rabies_canine, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_cml <-
    ggplot(rabies_camels, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_cttl <-
    ggplot(rabies_cattle, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                   species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_gts <-
    ggplot(rabies_goats, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                  species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  rabies_shp <-
    ggplot(rabies_sheep, aes(fill = inc)) + geom_sf() + theme_bw() + facet_grid(. ~
                                                                                  species) +
    scale_fill_gradient(
      low = "#ffffb2",
      high = "#b10026",
  ...
  plot2 <-
    gridExtra::grid.arrange(rabies_canine,
                            rabies_feline,
                            rabies_cttl,
                            rabies_cml,
                            rabies_gts,
  ...
  ggsave("rabies_animals.png",
         plot2,
         width = 18,
         height = 9)
  ggsave("rabies_human.png",
         rabies_h,
         width = 7,
         height = 7)
  ...
  all_zoonoses1b <- rbind(human_zoonoses, animal_zoonoses) %>%
    mutate(diseases = recode(diseases, "Brucella" = "Brucellosis", "brucellosis" =
                               "Brucellosis")) %>%
    ...
    distinct() %>%
    mutate(total1 = ifelse(
      diseases %in% "Brucellosis" &
        type %in% "Animal",
      total * 8486.25,
      total
    ))
  ...
  ggplot(all_zoonoses1b[all_zoonoses1b$diseases %in% "Brucellosis" , ],
         aes(
           x = date,
           y = total1,
           group = type,
    ...
    geom_smooth() + theme_bw() + scale_x_date(date_labels = "%b-%y", date_breaks =
                                                "6 months") +
    labs(x = "Period", y = "Frequency", color = "") + theme(text = element_text(size =
                                                                                  16))
  
  ggsave("Brucellosis_trend.png",
         width = 16,
         height = 8)
  ...
  all_zoonoses1b <- rbind(human_zoonoses, animal_zoonoses) %>%
    ...
    dplyr::select(date, diseases, type, total) %>%
    distinct() %>%
    mutate(total1 = ifelse(diseases %in% "Rvf" &
                             type %in% "Animal", total * 23.167, total))
  
  
  
  ggplot(all_zoonoses1b[all_zoonoses1b$diseases %in% "Rvf" , ],
         aes(
           x = date,
           y = total1,
           group = type,
  ...
  all_zoonoses1b <- rbind(human_zoonoses, animal_zoonoses) %>%
    ...
    distinct() %>%
    mutate(total1 = ifelse(
      diseases %in% "Anthrax" & type %in% "Animal",
      total * 93.667,
      total
    ))
  ...
  ggplot(all_zoonoses1b[all_zoonoses1b$diseases %in% "Anthrax" , ],
         aes(
           x = date,
           y = total1,
           group = type,
  ...
  all_zoonoses1b <- rbind(human_zoonoses, animal_zoonoses) %>%
    ...
    dplyr::select(date, diseases, type, total) %>%
    distinct() %>%
    mutate(total1 = ifelse(diseases %in% "Rabies" &
                             type %in% "Animal", total * 26.529, total))
  ...
  ggplot(all_zoonoses1b[all_zoonoses1b$diseases %in% "Rabies" , ]) +
    ...
    geom_smooth() + theme_bw() + labs(x = "Period", y = "Frequency", color =
