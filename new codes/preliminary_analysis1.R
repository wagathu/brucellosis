...
setwd("C:/Users/SKahariri/OneDrive - CGIAR/ILRI/PHD/DATA/R_phd")
...
human_zoonoses <- read_csv("Zoonosis_2014_2021_county.csv")%>%
  ...
  mutate(diagnosis=ifelse(diseases%in%"Brucella", "Lab confirmed", "Clinically confirmed"))%>%
...
animal_zoonoses_comb <- Animaldata%>%
  rbind(Animaldata_21) %>%
  mutate(year_1=ifelse(year_1==21,2021,year_1))
...
animal_zoonoses <- animal_zoonoses_comb%>%
  ...
  mutate(date=lubridate::dmy(paste0("01","-", month_2, "-",year_1)))%>%
...
animal_zoonoses <- animal_zoonoses%>%
  ...
  select(date, county, disease_condition,diagnosis, species,no_sick )#%>%
write_csv(animal_zoonoses, "animal_zoonoses.csv")
  distinct()%>%
...
##create dataset with average cases
all_zoonoses1a<- rbind(human_zoonoses, animal_zoonoses)%>%
  mutate(diseases=recode(diseases, "Brucella"="Brucellosis", "brucellosis"="Brucellosis"))%>%
  group_by(diseases, type, county)#%>%

write_csv(all_zoonoses1a, "all_zoonoses_counts.csv")

all_zoonoses1b <- all_zoonoses1a%>% 
  mutate(avg=mean(disease_values,na.rm=T))%>%
  ungroup()%>%
  dplyr::select(county, diseases, type,avg)%>%
  distinct()
  
write_csv(all_zoonoses1b, "all_zoonoses_avg.csv")
...
data("DataCatalogue")
# Extracting the table 2.2 in Volume 1 from rKenyaCensus
population<- V1_T2.2%>%
  filter(County!="Total")%>%
...
##add population column to count dataset
all_zoonoses1c<- full_join(all_zoonoses1a, population, by=c("county"="County"))

##compute incidence for humans
all_zoonoses1b$inc<- ifelse(all_zoonoses1c$type%in%"Human",
                            round(all_zoonoses1c$disease_values/all_zoonoses1c$Total*100000), NA)

##compute total cases
all_zoonoses1<- all_zoonoses%>%
  mutate(diseases=recode(diseases, "Brucella"="Brucellosis", "brucellosis"="Brucellosis"))%>%
  group_by(date, diseases, diagnosis, type)%>%
  mutate(all=round(sum(disease_values, na.rm=T)))%>%
  ungroup()%>%
  dplyr::select(-disease_values, -county)%>%
  distinct()

###############################################
ggplot(all_zoonoses1, aes(x=date, y=all, fill=diseases))+geom_col()+facet_grid(type~diagnosis, scales = "free_y")+theme_bw()+
...
county_rvf1<- full_join(county, all_zoonoses1b[all_zoonoses1b$diseases%in%"Rvf" & 
                                                         all_zoonoses1b$type%in%"Human",], by=c("COUNTY"="county"))
county_rvf1$diseases<-"Rift Valley Fever"
rvf_h<-ggplot(county_rvf1, aes(fill=inc))+geom_sf()+theme_bw()+facet_grid(~diseases)+
  scale_fill_gradient(low="#ffffb2", high="#b10026", na.value = "white")+
  labs(fill="Average cases \n per 100,000 people")+theme(text=element_text(size=16))

## Rabies

county_rabies1<- full_join(county, all_zoonoses1b[all_zoonoses1b$diseases%in%"Rabies" & 
                                                 all_zoonoses1b$type%in%"Human",], by=c("COUNTY"="county"))
rabies_h<-ggplot(county_rabies1, aes(fill=inc))+geom_sf()+theme_bw()+
  scale_fill_gradient(low="#ffffb2", high="#b10026", na.value = "white")+
  labs(fill="Average cases \n per 100,000 people")
