...
county_desc_cases <- df_incidence2 |> 
  select(county, contains('cases')) |> 
  group_by(county) |> 
  summarise(across(contains('cases'), ~sum(., na.rm = T)))

county_desc_incidence <- df_spatial |> 
  select(county, contains('incidence')) |> 
  group_by(county) |> 
  summarise(across(contains('incidence'), ~sum(., na.rm = T)))

county_desc_incidence_combined <- df_spatial_cum |> 
  group_by(county) |> 
  summarise(across(contains('incidence'), ~sum(., na.rm = T)))

fwrite(county_desc_cases, 'county_desc_cases.csv')
fwrite(county_desc_incidence, 'county_desc_incidence.csv')
fwrite(county_desc_incidence_combined, 'county_desc_incidence_combined.csv')
